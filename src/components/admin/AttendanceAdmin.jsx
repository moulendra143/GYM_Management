    ...
    const loadAttendance = async () => {
      try {
        ...
        setAttendanceData(
          data.map((item) => ({
            ...item,
            name: item.memberName || item.member || `Member ${item.memberId}`,
            username: item.memberUsername || "",
            timeIn: item.checkedInAt ? new Date(item.checkedInAt).toLocaleTimeString() : "",
            date: item.checkedInAt ? new Date(item.checkedInAt).toLocaleDateString() : "",
          }))
        );
      } catch (error) {
